...
type memoSweepStubQuerier struct {
	...
	rows    []db.ListUnregisteredSmartAccountsRow
	listErr error

	setErr     error
	setCalls   int
	lastParams db.SetSmartAccountMemoRegistrationParams
}

func (s *memoSweepStubQuerier) ListUnregisteredSmartAccounts(_ context.Context) ([]db.ListUnregisteredSmartAccountsRow, error) {
	return s.rows, s.listErr
}

func (s *memoSweepStubQuerier) SetSmartAccountMemoRegistration(_ context.Context, arg db.SetSmartAccountMemoRegistrationParams) error {
	s.setCalls++
	s.lastParams = arg
	return s.setErr
}
...
func TestMemoSweepRun_RegistersSuccessfully(t *testing.T) {
	...
	stub := &memoSweepStubQuerier{rows: []db.ListUnregisteredSmartAccountsRow{
	...
	assert.Equal(t, "CADDR", stub.lastParams.SmartAccountAddress)
...
func TestMemoSweepRun_SetSmartAccountMemoRegistrationError_CountsFailed(t *testing.T) {
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusOK)
		_ = json.NewEncoder(w).Encode(map[string]string{"memo_id": "999", "pool_address": "GPOOL"})
	}))
	...
	uid := uuid.MustParse("a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a11")
	stub := &memoSweepStubQuerier{
		rows:   []db.ListUnregisteredSmartAccountsRow{{UserID: uid, SmartAccountAddress: "CADDR"}},
