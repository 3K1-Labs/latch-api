...
// bound to a different wallet/key_type/network.
...
// Issue creates a random nonce bound to (wallet, keyType, network) and returns
// its hex encoding plus the TTL.
func (s *WalletNonceService) Issue(ctx context.Context, wallet, keyType, network string) (string, time.Duration, error) {
	...
	if err := s.redis.Set(ctx, walletNoncePrefix+nonceHex, bind(wallet, keyType, network), walletNonceTTL).Err(); err != nil {
